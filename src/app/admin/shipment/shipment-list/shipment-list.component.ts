...
export class ShipmentListComponent {
      ...
      importDate: {
        ...
        valuePrepareFunction: (date) => {
          if (date) {
            ...
            const day = raw.getDate() < 10 ? "0" + raw.getDate() : raw.getDate();
            const month = (raw.getMonth() + 1) < 10 ? "0" + (raw.getMonth() + 1) : raw.getMonth();
  ...
  // Constructor
  constructor(private shipmentService: ShipmentService, private router: Router, private toast: Toast) {
  ...
  loadShipmentData() {
    this.shipmentService.getShipmentsByBranch().subscribe(
      (res) => {
        if (res.code === 200) {
